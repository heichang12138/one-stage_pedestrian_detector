...
def multi_anchor_target_layer(rpn_cls_score_C5, rpn_cls_score_C6,
                              gt_boxes, dontcare_areas, im_info,
                              multi_feat_stride = [16, 32],
                              anchor_scales_C5 = [4 ,8, 16, 32],
                              anchor_scales_C6 = [4 ,8, 16, 32]):
    # get scale factor
    im_info = im_info[0]

    if cfg.SDS.SDS_ON:
        height, width = rpn_cls_score_C5.shape[1:3]
    ...
    rpn_blobs = [[], []]
    fg_num = 0.0
    for ci, (anchor_scales, _feat_stride, rpn_cls_score) in \
        enumerate(zip([anchor_scales_C5, anchor_scales_C6], multi_feat_stride, [rpn_cls_score_C5,rpn_cls_score_C6])):
        # generate base anchors
        _anchors = generate_anchors(scales=np.array(anchor_scales))
        _num_anchors = _anchors.shape[0]
        # tile a specific feature map
        ...
        # discard anchors cross image boundaries
        _allowed_border =  0
...
#        if not cfg.RETINA.RETINA_ON:
        num_fg = int(cfg.TRAIN.RPN_FG_FRACTION * cfg.MULTISCALE.BATCHSIZE[ci])
        ...
        num_bg = cfg.MULTISCALE.BATCHSIZE[ci] - np.sum(labels == 1)
        bg_inds = np.where(labels == 0)[0]
        if len(bg_inds) > num_bg:
            disable_inds = npr.choice(
                bg_inds, size=(len(bg_inds) - num_bg), replace=False)
            labels[disable_inds] = -1
        fg_num += np.sum(labels==1)
        ...
        rpn_blobs[ci].append(rpn_labels)
        rpn_blobs[ci].append(rpn_bbox_targets)
        rpn_blobs[ci].append(rpn_bbox_inside_weights)

    rpn_labels_C5 = rpn_blobs[0][0]
    rpn_bbox_targets_C5 = rpn_blobs[0][1]
    rpn_bbox_inside_weights_C5 = rpn_blobs[0][2]
    rpn_labels_C6 = rpn_blobs[1][0]
    rpn_bbox_targets_C6 = rpn_blobs[1][1]
    rpn_bbox_inside_weights_C6 = rpn_blobs[1][2]
    fg_num = np.float32(fg_num)
